		...
		PainelAdminController.prototype.getViagens = function(pessoa){
			...
			self.httpService.get('viagens').then(function(res) {
				for (var i = 0; i < res.data.length; i++) {
					res.data[i].veiculo = JSON.parse(res.data[i].veiculo); 
					res.data[i].passageiros = JSON.parse(res.data[i].passageiros); 
					res.data[i].motorista = JSON.parse(res.data[i].motorista); 
				}
